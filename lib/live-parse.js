...
function parseBox(d, fieldNorm) {
  return (((d.boxscore || {}).players) || [])
    .map((g) => {
      const out = { teamId: matchTeamId(g.team, fieldNorm), name: (g.team && (g.team.displayName || g.team.location)) || "?", batting: null, pitching: null };
      (g.statistics || []).forEach((st) => {
        const cols = pickCols(st.labels, st.type === "batting" ? BAT_COLS : PIT_COLS);
        const rows = (st.athletes || [])
          .map((a) => ({ name: (a.athlete && (a.athlete.shortName || a.athlete.displayName)) || "?", stats: cols.map((c) => (a.stats || [])[c.idx]) }))
          .filter((r) => r.name !== "?");
...
function parseSituation(d, state) {
  ...
  const nm = (x) => (x && x.athlete && (x.athlete.shortName || x.athlete.displayName)) || null;
...
export function parseSummary(d, eventId, fieldNorm) {
  ...
  const teams = (comp.competitors || []).map((c) => ({
    id: matchTeamId(c.team, fieldNorm),
    name: (c.team && (c.team.displayName || c.team.location)) || "?",
  ...
  return { id: eventId, state, detail: st.shortDetail || st.detail || "", teams, demo: false, situation: parseSituation(d, state), scoringPlays: parseScoringPlays(d), box: parseBox(d, fieldNorm) };
}
