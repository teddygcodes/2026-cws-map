...
/* Tournament date window for the ESPN scoreboard poll.
   Previously a hardcoded ["20260529".."20260601"] array that silently stopped
   covering games once the calendar passed June 1 (super-regionals + CWS would
   never appear). This computes a rolling window instead: a few days back (so a
   game suspended/finished yesterday still resolves) through several days ahead,
   clamped to the tournament's overall span. ESPN returns an empty event list
   for off-days, which the caller already handles, so an over-wide window is
   harmless. Pure + injectable `now` for deterministic tests. */
const TOURNEY_START = "20260529"; // regionals first pitch
const TOURNEY_END = "20260623"; // generous CWS finals backstop (NCAA finals ~mid-late June)
function ymd(d) {
  return d.getFullYear() + String(d.getMonth() + 1).padStart(2, "0") + String(d.getDate()).padStart(2, "0");
}
export function tourneyDates(now, back, ahead) {
  const base = now instanceof Date ? now : new Date();
  const lookBack = back == null ? 1 : back;
  const lookAhead = ahead == null ? 6 : ahead;
  const out = [];
  for (let i = -lookBack; i <= lookAhead; i++) {
    const d = new Date(base.getFullYear(), base.getMonth(), base.getDate() + i);
    const key = ymd(d);
    if (key >= TOURNEY_START && key <= TOURNEY_END) out.push(key);
  }
  // Before the tournament opens, still poll opening day so a pre-event visit
  // shows the schedule rather than nothing.
  if (!out.length) {
    const today = ymd(base);
    if (today < TOURNEY_START) out.push(TOURNEY_START);
  }
  return out;
}
