...
- **🏆 Private leagues** *(optional)* — compete with friends on a shared league code with a live leaderboard, locked at first pitch. This is the one feature with a backend: a tiny Cloudflare Worker + KV (all scoring still happens in the browser). It's **off by default** and the rest of the app is unaffected; see [`worker/README.md`](worker/README.md) to deploy it and flip it on.
