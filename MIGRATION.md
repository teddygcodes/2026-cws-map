...
| Auth.js v5 config | `auth.js` (Google + Resend email link, pg adapter, database sessions) |
...
### 2. Resend (passwordless email link)
- [resend.com](https://resend.com) → create an account.
- Add a sending domain (verify SPF + DKIM in your DNS).
- Generate an API key.
- Set `AUTH_EMAIL_FROM` to an address on the verified domain
  (default in `auth.js` is `no-reply@cws-map.app` — change it).

### 3. Vercel project + Postgres
...
### 4. Project environment variables (Vercel → Settings → Environment Variables)

```
AUTH_SECRET        # generate with: npx auth secret
AUTH_GOOGLE_ID     # from step 1
AUTH_GOOGLE_SECRET # from step 1
AUTH_RESEND_KEY    # from step 2
AUTH_EMAIL_FROM    # the verified-domain sender from step 2
...
### 5. Add the Vercel origin(s) to the Worker CORS allow-list
